...
{
    s_role = role;
    ESP_LOGW(TAG, "init role=%d (stub — wire NimBLE mesh provisioning + vendor model)",
             (int)role);
    // TODO:
...
esp_err_t dp_mesh_publish_sample(const dp_radar_sample_t *s)
{
    ...
    if (s_role != DP_MESH_ROLE_SENSOR) return ESP_ERR_INVALID_STATE;
    ESP_LOGI(TAG, "publish presence=%d distance_cm=%u (stub)",
             s->presence, s->distance_cm);
...
{
    if (s_role != DP_MESH_ROLE_GATEWAY) return ESP_ERR_INVALID_STATE;
    s_handler = cb;
    return ESP_OK;
}
